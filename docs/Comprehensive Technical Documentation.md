...
- **Layer 2 — a trained ML classifier, wired into the live pipeline as a safety net.** It only ever activates when Layer 1 finds nothing at all, and it only ever *flags a prompt for review* — it can never mask anything itself. Section 8 documents this integration in full, including an honest, evaluated finding that the classifier alone overfits to synthetic-dataset structure, and the structural-evidence gate built to correct for it before this layer was allowed to ship.

**Current measured accuracy** (`evaluate.py` against a 5,012-prompt synthetic dataset): **Precision 95.1% / Recall 93.2% / F1 94.1%**, a **95.9% adversarial-obfuscation detection rate**, a **0.0% edge-case false-positive rate**, and — evidence the ML layer is doing genuine work, not decoration — **123 prompts where Layer 1 found nothing but Layer 2 correctly flagged real sensitive content for review, with a verified 0% false-alarm rate on a genuinely held-out out-of-distribution probe set** (Section 8.3, Section 10.4).
...
3. **Layer 2 is bounded so it cannot become a new liability.** It only runs when Layer 1 found *nothing*, it never masks, and — after an evaluated false-positive finding (Section 8.3) — it requires independent structural corroboration before it is allowed to surface anything at all. A security enhancement that itself becomes a source of noise or false confidence is not a net improvement; this project treats that as a real risk to design against, not an acceptable cost of "having ML."
...
The live web demo (`web/server.py` → `web/app.js`) surfaces this directly: every `/api/detect` response includes an `ml_safety_net` field, and the UI shows a distinct banner — *"no rule-based pattern matched, but the trained classifier scored this prompt N% likely sensitive by structure/entropy — flagged for review, not masked"* — whenever Layer 2 fires, separate from and never overlapping with Layer 1's masked-entity cards. A panel member typing a prompt into the live demo can see both layers acting independently, in real time, with the CLI/`main.py` and `evaluate.py` reporting the same signal for consistency.

### 8.5 Honest boundaries of this integration

- Layer 2 is prompt-level (a whole-prompt "does this look sensitive" signal), not span-level — it cannot say *which part* of a prompt looks anomalous, only that something does. Pinpointing the span is exactly what the research track's Task B (CRF token-level tagging) targets, and it is **not** wired into the live pipeline: its training labels are derived from an entropy heuristic rather than true entity-position ground truth (`build_notebook.py` Section 8), a labeling-quality gap that needs to close before it could be trusted for even a review-only role the way the classifier now is.
...
This is reported honestly rather than folded into the headline precision/recall numbers, because Layer 2 never masks anything — its 123 "true misses caught" are not counted as Layer 1 detections. Unlike the earlier retrain attempt, this 0-false-alarm figure is corroborated by a genuinely held-out signal, not just in-distribution structure: `research/ood_probe_set.json`'s 40 never-trained-on sentences independently show a 0% gated false-positive rate too (Section 8.3). The structural-evidence gate remains the mechanism that guarantees this in production, not a substitute for it.
...
| ML safety-net tests | `tests/test_ml_anomaly.py` | Verifies the structural-evidence gate, that Layer 2 can never mask anything or run when Layer 1 already did, and (`TestOodProbeSet`) that the shipped model + gate combination never flags any of the 40 held-out out-of-distribution probe sentences |
